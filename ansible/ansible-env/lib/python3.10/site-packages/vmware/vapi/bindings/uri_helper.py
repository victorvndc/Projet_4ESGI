...
__copyright__ = 'Copyright 2015, 2017, 2023 VMware, Inc.  All rights reserved. -- VMware Confidential'  # pylint: disable=line-too-long
...
class URIValidator(object):
    ...
    for rule_type in ['IRI', 'IRI_reference']:
        ...
        if regex is None:
            ...
            regex_wrap = 'r"""{0}"""'
            if six.PY2:
                # PY2 is utilized by buildweb and the legacy `re` module produces errors on raw backslashes
                regex_wrap = 'u"""{0}"""'

            regex_str = ast.literal_eval(regex_wrap.format(regex_str))
