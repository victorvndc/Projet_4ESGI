...
class ApiService(VapiInterface):
    ...
    def update(self,
        ...
        """
        Update the configuration of the NSX API service. Changes are applied to
...
class ApiVirtualIp(VapiInterface):
    ...
    def clearvirtualip(self):
        ...
        Note, query parameter **?action=clear_virtual_ip** clears virtual IPv4
        address and **?action=clear_virtual_ip6** clears virtual IPv6 address.
    ...
    def setvirtualip(self,
        ...
        """
        Sets the cluster virtual IPv4 and IPv6 address. Note, all nodes in the
        management cluster must be in the same subnet. If not, a 409 CONFLICT
        status is returned.
        Query parameter **ip_address** sets virtual IPv4 address and
        **ip6_address** sets virtual IPv6 address; either or both of the
        parameters can be specified to set virtual IP address(es).
        When updating either of any one parameter value this does not changes
        the value of other unspecified parameter.
