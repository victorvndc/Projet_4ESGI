...
class Certificates(VapiInterface):
    ...
    """
    APPLYCERTIFICATE_SERVICE_TYPE_ANALYTICS_AGENT = "ANALYTICS_AGENT"
    """
    Possible value for ``serviceType`` of method
    :func:`Certificates.applycertificate`.

    """
    APPLYCERTIFICATE_SERVICE_TYPE_ANALYTICS_KAFKA = "ANALYTICS_KAFKA"
    """
    Possible value for ``serviceType`` of method
    :func:`Certificates.applycertificate`.

    """
    APPLYCERTIFICATE_SERVICE_TYPE_NAPP_COMMON_AGENT = "NAPP_COMMON_AGENT"
    """
    Possible value for ``serviceType`` of method
    :func:`Certificates.applycertificate`.

    """
    APPLYCERTIFICATE_SERVICE_TYPE_NAPP_PACE_AGENT = "NAPP_PACE_AGENT"
    """
    Possible value for ``serviceType`` of method
    :func:`Certificates.applycertificate`.

    """
    APPLYCERTIFICATE_SERVICE_TYPE_NAPP_METRICS_AGENT = "NAPP_METRICS_AGENT"
    """
    Possible value for ``serviceType`` of method
    :func:`Certificates.applycertificate`.
    ...
    def applycertificate(self,
        ...
        designate the node where the certificate needs to be applied. Note that
        when applying CA-signed certificates to either the API certificate
        profile or the MGMT_CLUSTER certificate profile, the cerficate must
        have its CN or SAN extensions matching the endpoint's IP or FQDN. This
        also means that applying a CA-signed certificate to the MGMT_CLUSTER
        service profile to a cluster without first configuring its VIP is not
        allowed.
...
class Csrs(VapiInterface):
    ...
    def selfsign(self,
        ...
        signed with NSX's own private key. The maximum validity limit for
        non-CA certificates is 825 days, except for a value of 36,500 days,
        which should only be used for the cluster boot manager certificate. No
        limit is set for CA certificates.
    ...
    def selfsign_0(self,
        ...
        then self-sign it. The maximum validity limit for non-CA certificates
        is 825 days, except for a value of 36,500 days, which should only be
        used for the cluster boot manager certificate. No limit is set for CA
        certificates.
...
class CsrsExtended(VapiInterface):
    ...
    def create(self,
        ...
        :raise: :class:`com.vmware.vapi.std.errors_client.NotFound` 
            Not Found
        """
        warn('com.vmware.nsx.trust_management.CsrsExtended.create is deprecated.', DeprecationWarning)
        return self._invoke('create',
                            {
                            'csr': csr,
                            })
...
class StaleCertificates(VapiInterface):
    """
    
    """

    _VAPI_SERVICE_ID = 'com.vmware.nsx.trust_management.stale_certificates'
    """
    Identifier of the service in canonical form.
    """
    def __init__(self, config):
        ...
        :param config: Configuration to be used for creating the stub.
        """
        VapiInterface.__init__(self, config, _StaleCertificatesStub)
        self._VAPI_OPERATION_IDS = {}


    def list(self):
        """
        Get list of certificates that are currently not applied to any
        certificate profile and ones that are applied to deprecated certificate
        profiles.


        :rtype: :class:`com.vmware.nsx.model_client.StaleCertificatesListResult`
...
class _StaleCertificatesStub(ApiInterfaceStub):
    def __init__(self, config):
        # properties for list operation
        list_input_type = type.StructType('operation-input', {})
        list_error_dict = {
        ...
        list_rest_metadata = OperationRestMetadata(
            http_method='GET',
            url_template='/api/v1/trust-management/stale-certificates',
            path_variables={
            },
             header_parameters={
             },
        ...
        operations = {
            'list': {
                'input_type': list_input_type,
                'output_type': type.ReferenceType('com.vmware.nsx.model_client', 'StaleCertificatesListResult'),
                'errors': list_error_dict,
                'input_value_validator_list': list_input_value_validator_list,
                'output_validator_list': list_output_validator_list,
                'task_type': TaskType.NONE,
        ...
        ApiInterfaceStub.__init__(
            self, iface_name='com.vmware.nsx.trust_management.stale_certificates',
            config=config, operations=operations, rest_metadata=rest_metadata,
            is_vapi_rest=False)
...
class StubFactory(StubFactoryBase):
    _attrs = {
        ...
        'StaleCertificates': StaleCertificates,
        'TokenPrincipalIdentities': TokenPrincipalIdentities,
        'certificates': 'com.vmware.nsx.trust_management.certificates_client.StubFactory',
