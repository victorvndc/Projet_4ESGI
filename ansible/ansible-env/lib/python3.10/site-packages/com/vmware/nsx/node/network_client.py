...
class Interfaces(VapiInterface):
    ...
    def list(self):
        ...
        broadcast and host IP addresses, link and admin status, MAC address,
        network mask, and the IP configuration method (static or DHCP).
    ...
    def update(self,
        ...
        configure a controller or an edge node. **Note:** NSX manager reboot is
        required after adding IPv6 address.
...
class NameServers(VapiInterface):
    ...
    def update(self,
        ...
        associated with given domain names. If DHCP is configured, this method
...
class Routes(VapiInterface):
    ...
    def delete(self,
        ...
        """
        Delete a route from the node routing table. You can modify an existing
        route by deleting it and then posting the modified version of the
        route. To verify, remove the route ID from the URI, issue a GET
    ...
    def list(self):
        ...
        table. Routes can be of any type i.e. IPv4 or IPv6 or both. Route
        information includes the route ipv6 flag (True or False), route type
        (default, static, and so on), a unique route identifier, the route
        metric, the protocol from which the route was learned, the route source
        (which is the preferred egress interface), the route destination, and
        the route scope. If ipv6 flag is True then route information is for
        IPv6 route else for IPv4 route. The route scope refers to the distance
        to the destination network: The \"host\" scope leads to a destination
        address on the node, such as a loopback address; the \"link\" scope
        leads to a destination on the local network; and the \"global\" scope
...
class SearchDomains(VapiInterface):
    ...
    def get(self):
        """
        Returns the domain list that the node uses to complete unqualified host
        names. When a host name does not include a fully qualified domain name
        (FQDN), the NSX Management node appends the first-listed domain name to
        the host name before the host name is looked up. The NSX Management
        node continues this for each entry in the domain list until it finds a
        match.
    ...
    def update(self,
        ...
        unqualified host names. If DHCP is configured, this method returns a
